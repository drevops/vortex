(() => {
    ...
    return {
      ...
      updateGitHubRepo() {
        if (this.orgMachineName && this.siteMachineName) {
          this.githubRepository = this.orgMachineName + "/" + this.siteMachineName;
        }
      },
      updateContainerImage() {
        if (this.orgMachineName && this.siteMachineName) {
          this.databaseContainerImage = this.orgMachineName + "/" + this.siteMachineName + "-data:latest";
        }
      },
      updateWebRoot() {
        switch (this.hostingProvider) {
          ...
          case "none":
            ...
            break;
          case "other":
            if (this.webRootDirectory === "docroot" || this.webRootDirectory === "web") {
      ...
      // Utility functions
      toMachineName(str) {
        return str.toLowerCase().replace(/[^a-z0-9\s]/g, "").replace(/\s+/g, "_").replace(/_{2,}/g, "_").replace(/^_|_$/g, "");
      },
      toKebabCase(str) {
      ...
      toAbbreviation(str, maxLength) {
        ...
        return parts.map((part) => part.charAt(0)).join("").substring(0, maxLength);
      }
    };
  };
  ...
  function updateScaling() {
    ...
    const scaleFactor = minScale + (maxScale - minScale) * ((normalizedHeight - minHeight) / (maxHeight - minHeight));
