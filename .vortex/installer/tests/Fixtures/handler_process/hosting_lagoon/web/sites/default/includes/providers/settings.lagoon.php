...
if (!empty(getenv('LAGOON_KUBERNETES'))) {
  ...
  $lagoon_production_branch = getenv('VORTEX_LAGOON_PRODUCTION_BRANCH');
  ...
  if (getenv('LAGOON_ENVIRONMENT_TYPE') === 'production') {
    ...
    if (!empty($lagoon_git_branch) && !empty($lagoon_production_branch) && $lagoon_git_branch === $lagoon_production_branch) {
      $settings['environment'] = ENVIRONMENT_PROD;
    }
    // Dedicated test environment based on a branch name.
    elseif ($lagoon_git_branch === 'main' || $lagoon_git_branch === 'master') {
      $settings['environment'] = ENVIRONMENT_STAGE;
    }
    // Test environment based on a branch prefix for release and
    // hotfix branches.
    elseif (!empty($lagoon_git_branch) && (str_starts_with($lagoon_git_branch, 'release/') || str_starts_with($lagoon_git_branch, 'hotfix/'))) {
      $settings['environment'] = ENVIRONMENT_STAGE;
    }
  }
  ...
  $settings['cache_prefix']['default'] = (getenv('LAGOON_PROJECT') ?: getenv('VORTEX_PROJECT')) . '_' . (getenv('LAGOON_GIT_SAFE_BRANCH') ?: $lagoon_production_branch);
