...
[PHP TUI](https://phptui.dev/), where the questions are grouped into panels the
user moves between freely. At the same time, the CLI supports
**non-interactive installs** (via the `--no-interaction` flag) for both new and
existing projects, as well as updates — making it suitable for automated
pipelines and CI environments.

The CLI ships only the project-specific parts: the form definition in
`VortexForm` (the questions and their panels) and the handler classes that
carry the **Vortex** behavior. A single adapter converts a handler into a form
field; collection, conditionals and rendering belong to the library, which is
developed and released in its own repository. Discovery is split between the
two: each handler knows how to read its own value out of an existing project,
and the library decides when to ask for it.

The binary is `vortex` and it exposes five commands: `install`, `update`,
`configure`, `doctor` and `build`. Running it without a command resolves by the
state of the target directory — an existing **Vortex** project is reconfigured,
anything else gets a fresh install.
...
3. The form collects the answers, applying conditionals and discovery as it goes.
...
affect a hundred plus installation permutations, snapshot testing makes it easy
to **review the impact of a change across all scenarios as diffs** - ensuring
that regressions are caught before they reach consumers.
...
Commit your template and CLI changes **before** running
`ahoy update-snapshots` - the snapshots regenerate from the committed baseline,
so uncommitted changes are not picked up. `ahoy update-snapshots` is the only
sanctioned way to regenerate fixtures: invoking the test suite with
...
2. Generates an [Expect](https://en.wikipedia.org/wiki/Expect) script that
   answers the panels with paced keystrokes, typing at a human speed and
   pausing long enough for each screen to be read.
3. Records the session with `asciinema rec` into a JSON cast file.
4. Checks that the recorded run created the project, so a mistimed recording
   never reaches the published assets.
5. Post-processes the recording, replacing local paths with stable ones.
6. Converts the cast file to SVG and PNG using a custom
...
The script reaches each panel by counting arrow keys rather than by matching
text, because a panel redraws in place and a match would land on a line that
has already been overwritten. Adding, removing or reordering a panel therefore
means adjusting those counts in `build_install_expect_script()`.
