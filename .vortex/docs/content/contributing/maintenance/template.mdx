...
- Image modes download `seed.sh` from a pinned `drevops/mariadb-drupal-data` release. Renovate raises a pull request when a newer release is published, so the seeding behaviour only changes when that pull request is merged.
