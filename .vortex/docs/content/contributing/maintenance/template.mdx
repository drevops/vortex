...
BATS unit tests for shipped shell scripts live in the
[`drevops/vortex-tooling`](https://packagist.org/packages/drevops/vortex-tooling)
Composer package.
...
# Run a single test.
node_modules/.bin/bats .vortex/tooling/tests/unit/deploy.bats
```
...
- The `demo-image`, `test-image` and `all` modes download `seed.sh` from a pinned `drevops/mariadb-drupal-data` release and verify it against a recorded SHA-256 checksum.
- External versions are pinned as constants in the script and bumped by hand: `SEED_URL` together with `SEED_SHA256`, and `ALPINE_IMAGE` for the container that clears the SUT directory.
