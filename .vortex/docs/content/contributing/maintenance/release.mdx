...
- **`N.x`** - Development branch for the next major version, ahead of `main` (e.g., `2.x`)
- **`N-1.x`** - Maintenance branch for the previous major version, behind `main` (e.g., `1.x` after `2.x` is released)

When a new major version is ready:
1. The next major version development branch (`N.x`) is merged into `main`
2. The previous major version code is preserved in its own maintenance branch (`N-1.x`)
