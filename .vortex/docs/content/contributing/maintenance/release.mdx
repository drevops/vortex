...
Promoting a new major version is a single action: set the `VORTEX_CURRENT_MAJOR` repository variable to its number. The documentation default and the CLI's bare `/install` URL both follow it, while the per-major docs and `/v{N}/install` pins are always published.
...
11. Update install video with `cd .vortex && ahoy update-videos install`.
