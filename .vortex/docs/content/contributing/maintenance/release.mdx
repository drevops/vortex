...
- **`X.x`** - Development branch for the next major version, ahead of `main` (e.g., `2.x`)
- **`X.x`** - Maintenance branch for an older major version, behind `main` (e.g., `1.x` after `2.x` is released)

When a new major version is ready:
1. The next major version development in `X.x` is merged into `main`
