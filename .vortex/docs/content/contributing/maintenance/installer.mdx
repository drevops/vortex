...
repository as the template at `.vortex/installer/`. That way, a single pull
request can combine changes to the template and the installer, so template
modifications and the corresponding installer logic stay in sync.
...
touches many parts of the template and requires complex management of the
permutations of template variants. Using PHP allows the installer to provide a
terminal UI via [Laravel Prompts](https://github.com/laravel/prompts), where
users are guided through a series of questions to customize their project. At
the same time, the installer supports non-interactive installs (via the
`--no-interaction` flag) for both new and existing projects, as well as
updates. This makes it suitable for automated pipelines and CI environments.
...
2. **Environment variables** - using the `VORTEX_INSTALLER_PROMPT_<handler_id>`
   naming convention.
3. **Discovery** - auto-detection from existing project files (for example,
...
The installer handles replacements and additions with string substitution. It
also uses a token system to conditionally exclude entire blocks of content from
template files. This simplifies the management of optional features - rather
than requiring complex logic to surgically remove lines from configuration
files, scripts, or documentation, maintainers wrap the relevant content in
token markers. When a feature isn't selected during installation, the installer
removes everything between the markers.
...
assertions against the resulting files. A single template change can affect a
hundred plus installation permutations. Snapshot testing lets you
**review the impact of a change across all scenarios as diffs**, so regressions
are caught before they reach consumers.
...
- Each scenario directory (for example, `services_no_clamav/`,
  `hosting_acquia/`, `deploy_types_all_circleci/`) contains only the *delta*
...
- **Deployment**: artifact, webhook, Lagoon, all combined
...
logic changes, regenerate the snapshots from the `.vortex/` directory:
...
The regeneration *runs the installer for every permutation*, initiates a fresh
project for each scenario, and updates the fixture files to match the current
output. The resulting changes appear as diffs in version control, making it
straightforward to review exactly how a template or installer change affects
each scenario.

:::warning

Commit your template and installer changes **before** running
`ahoy update-snapshots` - the snapshots regenerate from the committed
baseline, so uncommitted changes aren't picked up. `ahoy update-snapshots` is
the only sanctioned way to regenerate fixtures: invoking the test suite with
`UPDATE_SNAPSHOTS` by hand bypasses part of the workflow and produces partial,
...
[Box](https://github.com/box-project/box). It's released automatically on each
**GitHub release** of the **Vortex** template and deployed to
https://www.vortextemplate.com/install - the [release process](release.mdx)
needs no separate installer step. Additionally, the installer is published for
every branch containing `release-docs` or `release-installer` in its name,
which allows testing installer changes before a formal release.

## Installer video

The documentation includes an interactive recording of the installer captured
with [asciinema](https://asciinema.org/). It's one of the 9 terminal demo
videos produced by the PHP orchestrator at
`.vortex/docs/.utils/update-videos.php`, driven by the shared `VideoRecorder`
class. For the installer video, the pipeline:
...
5. Converts the cast file to SVG and PNG using the custom
   `svg-term-render.js` script.
...
Re-record the installer video whenever the prompt flow changes - the recording
captures the live prompts. The command doesn't commit its output - review the
diff under `.vortex/docs/static/img/` and commit manually.

`sharp-cli`, which renders the PNG posters, is a declared dependency of
`.vortex/docs`, so it's pinned by `yarn.lock` and installed by the existing
