...
2. Add `NEWRELIC_ENABLED` (or `VORTEX_NOTIFY_NEWRELIC_ENABLED`) variable
   to your hosting provider's environment variables for environments where
   New Relic is configured. This prevents unnecessary API calls in environments
   without New Relic.
3. Add `VORTEX_NOTIFY_NEWRELIC_USER_KEY` variable to your hosting
   provider's global environment variables.
4. Optionally, add other New Relic-related variables to your `.env`
...
in development or feature branch environments that don't have New Relic set up.
...
| Variable                                | Required | Default                                       | Location | Description                                                       |
|-----------------------------------------|----------|-----------------------------------------------|----------|-------------------------------------------------------------------|
| `VORTEX_NOTIFY_NEWRELIC_ENABLED`        | **Yes**  | `NEWRELIC_ENABLED`                            | Hosting  | Enable New Relic notifications (set to `1` or `true`)             |
