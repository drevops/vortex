...
- **`release/*`** - release preparation branches (for example, `release/25.1.0`)
- **`hotfix/*`** - emergency fixes for production (for example, `hotfix/25.1.1`)

<details>
  <summary><code>production</code> branch</summary>
  ...
  so tag-based deployments aren't supported. In this case, you must create a
...
1. **Start the release**
   ...
   Creates a `release/X.Y.Z` branch from `develop`. We recommend pushing the
...
4. **Deploy to production**
   ...
   - **Branch-based hosting (for example, Lagoon):** manually sync to the `production` branch
...
5. GitHub has no open PRs related to the release
