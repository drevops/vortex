...
| `VORTEX_FETCH_DB_LAGOON_SSH_HOST` | Lagoon SSH host (default: `ssh.lagoon.amazeeio.cloud`) |
| `VORTEX_FETCH_DB_LAGOON_SSH_PORT` | Lagoon SSH port (default: `32222`) |
...
- `VORTEX_DB_SSH_KEY` - key with read access, for database fetches.
...
`VORTEX_FETCH_DB_SSH_KNOWN_HOSTS` (database fetch) and
...
`db_ssh_fingerprint` (database fetch) and `deploy_ssh_fingerprint` (deployment).
