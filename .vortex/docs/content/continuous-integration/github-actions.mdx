...
| `release/**` | Release preparation branches (for example, `release/1.2.3`, `release/2023-04-17`) |
| `hotfix/**` | Urgent production fixes (for example, `hotfix/1.2.4`, `hotfix/2023-04-17`) |
...
| Pattern | Description |
|---------|-------------|
| All push branches listed earlier | Same as push triggers |
| `feature/**` | New feature branches (for example, `feature/login`, `feature/123-new-ui`) |
| `bugfix/**` | Non-urgent bug fix branches (for example, `bugfix/fix-auth`, `bugfix/456-crash`) |
...
Security checks live in a second workflow, `.github/workflows/audit.yml`. It
triggers on the same branches and tags as the main workflow. You can also start
it on demand from **Actions → Security audit → Run workflow**.
...
dumps, and every file created while provisioning share what's left.
...
anything else. In the preceding measurement, this freed about 15 GB in around
14 seconds, taking the job from roughly 13 GB of headroom to 28 GB.

The removal is destructive to the runner for the rest of the job, so it's off
by default. Turn it on by setting the `VORTEX_CI_FREE_DISK_SPACE` variable to
`1` in **Settings → Secrets and variables → Actions → Variables**. Leave it
unset on a project whose workflow runs a step that needs Haskell, Swift,
PowerShell, .NET, or CodeQL.

:::note

Every figure here comes from one runner image at one moment, and GitHub
...
With `VORTEX_CI_FREE_DISK_SPACE` enabled, the Android SDK is the single largest
remaining tree at about 10 GB, but removing it takes around 50 seconds. It
ships commented out in the workflow for that reason. To enable it, uncomment
the line in the **Free up disk space on the runner** step of both the `build`
and `database` jobs:
...
filesystem. It has to, because the job itself runs in a container and can't see
the host's directories directly. This is also why the general-purpose
disk-cleanup actions available on the Marketplace have no effect in a
**Vortex** job.
...
Set the `VORTEX_CI_BUILD_CACHE_IMAGE` variable in **Settings → Secrets and
variables → Actions → Variables** to reuse the `cli` image layers between
runs. See
[Container image build cache](./README.mdx#container-image-build-cache).
...
running the `lint`, `database`, or `build` jobs. Open the **Actions** tab, select
the `Database, Build, Test and Deploy` workflow, click **Run workflow**, and
provide:

| Input           | Description                                                                                    |
|-----------------|------------------------------------------------------------------------------------------------|
| `deploy_target` | Branch name (for example, `develop`) or PR reference (for example, `PR-123`, case-insensitive) |
...
environment is overridden with a fresh copy taken from the production
environment.

:::note

Artifact-based deployments don't work with manual deploys as the build job
is skipped.
...
PHPUnit and Behat results are published to the **Checks** tab of each run. A
failed check links directly to the failing tests and their messages, so you
don't need to download artifacts or search through logs. Results are read from
the JUnit reports the test tools already generate, so no extra setup is
required.
