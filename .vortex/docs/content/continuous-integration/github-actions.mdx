...
and [reviewdog](https://github.com/reviewdog/reviewdog) publishes each one as a
check. Every finding is annotated against its file and line in the pull request
diff, so a coding-standards failure no longer needs the job log to locate it.
The reports are also uploaded as the `lint-artifacts` artifact.

Rector annotates through its own GitHub output format and writes no report,
because it offers neither a checkstyle nor a JUnit formatter.

Each tool keeps its console output and its
`VORTEX_CI_<TOOL>_IGNORE_FAILURE` behavior. A pull request from a fork carries a
read-only token that cannot create a check, so there the publishing step reports
a warning and the job continues. To disable publishing, set
`VORTEX_CI_LINT_RESULTS_SKIP` to `1` in **Settings → Secrets and variables →
Actions → Variables**.
