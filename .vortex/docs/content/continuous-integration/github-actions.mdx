...
The `deploy` job adds the URL of the deployed environment to the run summary,
so the environment can be opened from the run page.

The URL is read from the environment's Lagoon route, so it appears only for
[Lagoon deployments](/docs/deployment/lagoon) and only once that route exists —
an environment created by the deployment itself has none until its first build
completes. Artifact and webhook deployments hand the code to the hosting
provider without learning a URL, so nothing is added for them.
