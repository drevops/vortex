...
`VORTEX_CI_<TOOL>_IGNORE_FAILURE` behaviour. Publishing is skipped on pull
requests from forks, where the workflow token cannot create a check. To disable
it everywhere, set `VORTEX_CI_LINT_RESULTS_SKIP` to `1` in **Settings → Secrets
