...
[Set up continuous integration](/docs/installation?ci-provider=circleci#7-set-up-continuous-integration)
...
patterns, controlled by a regular expression filter in the `workflows` section
of `.circleci/config.yml`:
...
| `ci*` | Temporary CI testing branches (for example, `ci`, `ci-debug`) |
| `N.x` | Major version branches (for example, `1.x`, `2.x`) |

To modify which branches trigger deployments, update the regular expression in
the `only` filter of the `deploy` job.
...
the next day. It's scheduled against the `develop` branch - change the branch
filter of the `nightly-db` workflow if your project doesn't use one.
...
Resource classes scale CPU and memory only. Disk space isn't tied to the
resource class, so upgrading it doesn't help a job that runs out of space -
see [Runner disk space](./README.mdx#runner-disk-space).

### Enable the build cache

Set `VORTEX_CI_BUILD_CACHE_IMAGE` in the `runner_config` environment of
`.circleci/config.yml`, or as a project environment variable, to reuse the
`cli` image layers between runs. See
