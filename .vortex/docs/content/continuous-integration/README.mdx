...
Continuous integration jobs run on hosted runners with a fixed amount of disk space - a fixed allocation on CircleCI, and a 72 GB root volume on the GitHub Actions `ubuntu-latest` runner. On GitHub Actions the preinstalled software occupies about 59 GB of that volume, so a job starts with around 13 GB free. Pulled container images, database dumps, and every file created while provisioning share what is left.
...
- On GitHub Actions, the `build` and `database` jobs remove the preinstalled toolchains no **Vortex** job uses - .NET, Swift, GHCup, PowerShell and CodeQL - before doing anything else. This frees about 15 GB in around 14 seconds, taking the job from roughly 13 GB of headroom to 28 GB.
- Those same two jobs print `df -h` in a **Report disk usage** step, so the disk state is on the record for every run.
...
#### Keep the preinstalled toolchains

Removing the toolchains is destructive to the runner for the rest of the job. A project whose workflow runs a step that needs .NET, Swift, Haskell, PowerShell or CodeQL should keep them by setting the `CI_FREE_DISK_SPACE` variable to `0` in **Settings → Secrets and variables → Actions → Variables**.

#### Free even more space

The Android SDK is the single largest remaining tree at about 10 GB, but removing it takes around 50 seconds. It ships commented out in the workflow for that reason - uncomment the line in the **Free up disk space on the runner** step of both the `build` and `database` jobs to enable it:
...
These figures come from one runner image at one point in time, and GitHub reissues that image regularly. The **Report disk usage** step is the check against drift - if the reclaimed amount shrinks, the path list needs revisiting.
