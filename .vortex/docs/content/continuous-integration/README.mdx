...
- On GitHub Actions, the `build` and `database` jobs remove the preinstalled toolchains no **Vortex** job uses - .NET, Swift, GHCup, PowerShell and CodeQL - before doing anything else. In the same measurement this freed about 15 GB in around 14 seconds, taking the job from roughly 13 GB of headroom to 28 GB.
...
Every figure above comes from one runner image at one point in time, and GitHub reissues that image regularly. Read the **Report disk usage** output of a recent run for the current numbers - if the reclaimed amount has shrunk, the path list needs revisiting.
