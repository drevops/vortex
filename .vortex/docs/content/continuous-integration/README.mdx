...
- Runs in parallel with other jobs (no dependencies)
- Needs no application containers and no installed project dependencies
- Scans the codebase for committed secrets with Gitleaks
- Audits the locked Composer packages against known security advisories
...
- Runs after successful completion of the `build`, `lint` and `security` jobs
...
Code linting and security checks run in the separate `lint` and `security` jobs
and are not affected by test parallelism settings.
