...
It doesn't change what the audit does. `composer audit` still reports the advisory and still fails the workflow until the advisory is added to `ignore-id` as described in [Ignoring](#ignoring). Dismiss the alert to record the assessment, and add the `ignore-id` entry to stop the build failing.
...
Dismissing an alert records the assessment but doesn't stop the check failing; that still needs the affected version removed from the tree, as described in [Ignoring](#ignoring-1).
