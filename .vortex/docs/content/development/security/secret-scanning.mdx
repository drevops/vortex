...
It doesn't change what the scan does. Gitleaks still matches the value and still fails the workflow until the line carries a `gitleaks:allow` comment or the value is allowlisted in `.gitleaks.toml`, as described in [Ignoring](#ignoring).
