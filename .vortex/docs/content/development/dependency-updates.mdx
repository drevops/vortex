...
**Vortex** provides [Renovate](https://renovatebot.com) configuration for your project's automated dependency updates.
...
requires 2 repository settings:
...
The configuration is stored in [`renovate.json`](https://github.com/drevops/vortex/blob/main/renovate.json). It's
based on the [Renovate configuration for automated Drupal dependency updates](https://github.com/drevops/renovate-drupal)
project.
...
- **Hourly PR limit:** `prHourlyLimit: 0` - no hourly cap; the per-group
  schedules listed earlier provide throttling.
...
If the job is configured but a required variable isn't provided, the job
...
- `RENOVATE_GIT_AUTHOR`: Author for Renovate commits in the format
  `Name <email@example.com>` (for example,
...
Triggering GitHub Actions from the self-hosted service isn't supported.
