...
On the Drupal side, the detected type is stored in `$settings['environment']`,
which modules and settings overrides use to drive environment-specific
behavior; shell scripts read the `ENVIRONMENT_TYPE` environment variable
instead. **Vortex** treats `preview` the same as `development` in its module
settings.
...
by requiring HTTP authentication credentials. **Vortex** enforces Shield in
non-production hosted environments, keeps local development and CI accessible
without authentication, and leaves production alone so Shield stays under UI
control there.
...
| Preview     | **Yes**        | Protects ephemeral preview environments                           |
...
| Preview     | **Yes**           | Prevents accidental delivery to real users                    |
| Stage       | No                | Stage may require real email delivery for UAT                 |
| Prod        | No                | Production emails must reach actual recipients                |

Rerouting is also enabled in any custom environments that do not match the
standard environment types listed above.
