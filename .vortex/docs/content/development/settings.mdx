...
environment, which [Shield](modules.mdx#shield) protects with HTTP
...
1. Test the behavior locally first: add `DRUPAL_SHIELD_DISABLED=1` to
   `.env.local` and re-create the containers with `ahoy up`.
2. Set the same variable on the `stage` environment only, using your hosting
   provider's environment variables (some providers need a redeploy or an
   environment restart for a new variable to reach the runtime) - see
   [Acquia](../hosting/acquia.mdx) and [Lagoon](../hosting/lagoon.mdx).
3. When the review is over, remove the variable to restore the default
   behavior.
...
When `DRUPAL_HASH_SALT` is not set, **Vortex** derives a fallback salt by hashing
the database host name. This lets a project start quickly and is suitable for
**local and CI use only**:

- A database host is not a secret - it appears in logs, backups, `.env` files,
...
It does not apply any configuration changes - those are handled separately in
...
➡️ See [Modules](modules.mdx) for what each shipped override configures, module by module.
...
- Provide default values for when environment variables are not set
...
Prefix all such variables with `DRUPAL_` (e.g. `DRUPAL_MY_SETTING`) to
...
Remove these tests if you do not want to maintain them.
