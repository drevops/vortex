...
flowchart LR
  ...
  H --> I[vr-compare resolves PR by SHA, checks label]
  I --> J[vr-report]
...
number**. The workflow itself looks up the associated open PR via the
GitHub API (using the commit SHA) and verifies the `VR` label is present
(case-insensitive). If no open PR matches the SHA, the run is skipped.
...
`VORTEX_VR_DIFFY_AUTO_BRANCHES`: a comma-separated glob list of PR head
...
`VORTEX_VR_DIFFY_AUTO_BRANCHES=deps/*,dependabot/*`. Set the variable to
empty to disable the bypass entirely (every PR, including bot PRs, then needs
the label).

### Default behaviour out of the box

With `VORTEX_VR_DIFFY_AUTO_BRANCHES=deps/*` (default) and
`VORTEX_VR_DIFFY_PR_LABEL=VR` (default):
...
`VORTEX_VR_DIFFY_AUTO_BRANCHES=deps/*,dependabot/*`. Consumers who want the
label as the only gate (no auto-bypass) set the variable to empty.

## How the PR is resolved

The workflow looks up the PR associated with the deployed commit SHA via
the GitHub API. Specifically:

```
gh pr list --search "<sha>" --state all --json number
```

This means:

- The deployed SHA can match an **open or closed** PR. A typical example:
  a feature PR is merged into `dev`, the host deploys the merge commit,
  the lookup finds the now-closed feature PR, and the comparison summary
  is posted there as a post-merge record.
- The lookup runs in GitHub Actions where `GITHUB_TOKEN` is available - no
  PAT or extra config required.
- The host does not need to provide a PR number; every host that runs the
  `notify-diffy` channel works identically.

If the SHA matches multiple PRs (unusual, e.g. duplicate commits from a
fork), the first result is used.

## Missed-window behaviour
...
4. Polls the comparison status, printing progress to the job log every
   `VORTEX_VR_DIFFY_POLL_INTERVAL` seconds.
