...
asks you to map the incoming connection to a project.

To use Xdebug on the project for the first time, follow these steps (assuming
you already have a fully working site):
...
6. PhpStorm stops on your breakpoint and asks you to map the incoming connection
   to a directory in your project. This is because the code runs in the
   container, which qualifies as a remote server. You need to "tell" Xdebug
   where to find the code on your local machine that corresponds to the code
   running in the container. You need to do it once, and PhpStorm remembers the
   mapping.

![PhpStorm server settings with the path mapping to the project root within the container](/img/xdebug-phpstorm.png)
...
- Increase the number in `Max. simultaneous connections` to `5` or more. This
  prevents a hidden debug session from being "stuck" without being visible in
  the IDE.
...
- **Cookie storage**: Always use `.data/` directory - it's gitignored
