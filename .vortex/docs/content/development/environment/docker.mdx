...
If you are using Lagoon as your hosting provider, then all your environments
(local, CI, and production) are using identical images to run the project.
...
The containers are started from _images_ - templates that define what's
installed in the container and how it's configured - so every container starts
...
container and the host. Changes made on the host are then immediately reflected
in the running application.

The file defines 2 volumes:
...
- Files directory `./web/sites/default/files` maps
  ...
  to optimize performance, because files aren't changed as often as the code.

The file defines 2 more volumes that are commented out - these are used in
...
The default user is defined as `1000` - this is the user ID used in the
...
1. Variables that are specific to the stack and require a default value (
   like `LAGOON_ROUTE`).
2. Variables that can't be stored in `.env` file and are injected from the
...
- `TZ: ${TZ:-UTC}` - defines a variable `TZ` with a default value of `UTC`, but
  only if `TZ` variable isn't defined.
- `LOCALDEV_URL: &default-url ${COMPOSE_PROJECT_NAME:-example-site}.docker.amazee.io` - defines
  ...
  `LOCALDEV_URL` variable isn't defined. The value of the variable
...
- `php` - a container that runs a PHP engine. This container is used to run the
  application code and execute commands in the context of the application. It's
  different from the `cli` container in that it doesn't have certain development
  dependencies installed, has a smaller size, and is optimized for scalability.
...
- `database2` - an optional second database container holding the source
  database for [migrations](../migrations.mdx). It's only present when
...
- `solr` - an optional container that runs an Apache Solr 9 search server to
  store the application search index and provide full-text search. It's built
...
- `chrome` - container that runs a Chrome browser. This container is
  used to run Behat tests. It's based on the official Selenium image which has
...
- `wait-for-dependencies` - a container that runs a script to wait for applications
  ...
  startup of containers, but it doesn't know when the application within the
...
that connects all containers together.

## Build context and `.dockerignore`

When an image is built, Docker sends a **build context** - the set of files it's
allowed to copy into the image - to the build engine. **Vortex** controls this
context with a `.dockerignore` file written as a **deny list**. Every file is
part of the build context by default, and the file lists only what must be kept
out.
...
After updating the `docker-compose.yml` file, it's sometimes useful to validate
