...
5. Add `scripts/provision-20-migration.sh`.

## Environment variables

| Variable | Default | Description |
|----------|---------|-------------|
| `VORTEX_FETCH_DB2_FILE` | `db2.sql` | Migration database dump filename |
...
tooling script with `VORTEX_DB_INDEX=2`. The index makes all scripts resolve
indexed variable names (for example, `VORTEX_FETCH_DB2_SOURCE` instead of
`VORTEX_FETCH_DB_SOURCE`, and `VORTEX_DB2_IMAGE` instead of `VORTEX_DB_IMAGE`).
As a result, all existing fetch sources (URL, FTP, Acquia, Lagoon, S3) work for
the migration database as well.

## Provision script

The `scripts/provision-20-migration.sh` script runs automatically
...
On projects with the Solr service, the script disables the search server before
the migrations and enables it again afterward, so migrated entities aren't
indexed one at a time. The `scripts/provision-30-search-index.sh` script rebuilds
the index straight after.

Set `DRUPAL_MIGRATION_SEARCH_DISABLE=0` to leave the search server running and
index entities as they're migrated.
...
If `DRUPAL_MIGRATION_SOURCE_DB_IMPORT` is `0`, the script probes the source database
...
4. Add your migrations to `scripts/provision-20-migration.sh`:
