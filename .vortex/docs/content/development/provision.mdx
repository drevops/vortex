...
The provisioning process sets up a Drupal site on an already assembled
codebase. It either imports an existing database from the dump or installs a
fresh instance of Drupal using a profile. It then runs the necessary
configuration import and database updates.

The `provision` script handles this process. It ships via the
[`drevops/vortex-tooling`](https://packagist.org/packages/drevops/vortex-tooling)
Composer package and is installed at `vendor/bin/vortex-provision`.
...
│  │                                              └─ Dump file missing?
│  │                                                  ├─ ④ Fallback? ──Yes──► 📦 Install from profile ✓ (then ⑤ = Yes)
...
1. `VORTEX_PROVISION_SKIP=1`<br/>Kill-switch to skip provisioning. The script exits immediately after start. Useful in emergencies when any kind of automation needs to be disabled.<br/><br/>
...
4. `VORTEX_PROVISION_FALLBACK_TO_PROFILE=1`<br/>Automatically fall back to installing from profile if the database dump file or container image isn't available. The site is installed from the configured profile, and the Shield module is enabled to protect the environment. All post-provision operations (configuration import, database updates, deployment hooks, etc.) are skipped. This provides a minimal working Drupal site when no database is available.<br/><br/>
...
6. `VORTEX_PROVISION_USE_MAINTENANCE_MODE=1`<br/>Enable maintenance mode right after the site is bootstrappable and disable it at the end. Useful when you want to prevent users from accessing the site while it's being provisioned.<br/><br/>
7. `VORTEX_PROVISION_VERIFY_CONFIG_UNCHANGED_AFTER_UPDATE=1`<br/>Verify that active configuration wasn't changed by database updates. When enabled and config files are present, the provision fails if `drush updatedb` modifies active configuration, preventing `drush config:import` from silently overwriting those changes.<br/><br/>
...
9. `VORTEX_PROVISION_CONFIG_IMPORT_REPEAT=1`<br/>Repeat the configuration import after the initial import. Useful when database update hooks introduce new configuration that affects later configuration imports (for example, new config_split settings). Disabled by default.<br/><br/>
10. `VORTEX_PROVISION_SANITIZE_DB_SKIP=1`<br/>Disable database sanitization.<br/><br/>
11. `VORTEX_PROVISION_VERIFY_CONFIG_UNCHANGED_AFTER_PROVISION=1`<br/>Verify that active configuration wasn't changed by the post-provision operations. When enabled and config files are present, the provision fails if any configuration item reported by `drush config:status` is in the `Different` state. The check runs after the configuration imports, deployment hooks, and custom provision scripts have finished. Items that exist only in the database or only in the sync directory aren't treated as a failure.
...
prevent users from accessing the site while it's being updated.
...
dump before it can be downloaded (sanitize on export). Tools such as
[Drush GDPR Dumper](https://github.com/robiningelbrecht/drush-gdpr-dumper)
or [MTK](https://github.com/skpr/mtk) are available for this purpose. These
tools can be integrated into **Vortex**-based projects without changing the
...
imported into the local environment. This allows for caching and reusing the
database dump, so you don't need to fetch it every time you refresh the local
environment.
...
You can load the dump from `.data` into the running database container in 2
distinct ways - pick the one matching what you need.
...
- **Supporting extensibility:** It allows for custom post-provisioning scripts, so you can layer in
...
`chmod +x scripts/provision-50-custom.sh`. The script is automatically
...
We recommend using a 2-digit suffix to control the order of execution - for
...
| `provision-50-storybook.sh`           | Builds and publishes the Storybook component library     | Storybook is selected; removed by the installer otherwise                                                   |
...
and switches the administration interface to the core Navigation module. It
also installs the contrib, service, and custom site modules before running
their deployment hooks.
...
and example scripts are adapted or removed. They're also the modules kept out
...
there. This includes the email rerouting that
...
Installing a module that's already installed does nothing, so provisioning a
...
site provisioned once with `DRUPAL_GENERATED_CONTENT_SKIP=1` doesn't gain the
content by unsetting the variable and provisioning again.

### Migration

Projects with the migration feature ship a `scripts/provision-20-migration.sh`
custom script that runs the content migration. It imports the migration source
database dump into the `database2` container, probes it for corruption, and
runs the migrations via `drush migrate:import`, with an optional rollback
before the import.

On projects with the Solr service, the script disables the search server while
the migrations run, so migrated entities aren't indexed one at a time. The
search indexing script rebuilds the index straight after.

➡️ See [Migrations](migrations.mdx#provision-script) for the script's
configuration variables and how to add your own migrations.
...
these environments receive a database with an index tracker state that doesn't
...
All these hooks run automatically during the
...
facade helpers for the common deploy hook operations. With them, a hook stays a
few lines instead of repeating the entity API boilerplate every time:
