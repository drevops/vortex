...
11. `VORTEX_PROVISION_VERIFY_CONFIG_UNCHANGED_AFTER_PROVISION=1`<br/>Verify that active configuration was not changed by the post-provision operations. When enabled and config files are present, the provision will fail if any configuration item reported by `drush config:status` is in the `Different` state once the configuration imports, deployment hooks and custom provision scripts have run. Items that exist only in the database or only in the sync directory are not treated as a failure.
...
the migrations run, so migrated entities are not indexed one at a time - the
