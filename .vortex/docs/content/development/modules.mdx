...
|-----------|----------|--------|
| Composer requirement | `require` in `composer.json` | Makes the module available to the site. ➡️ See [composer.json](composer.mdx#require) |
...
| Provisioning | `scripts/provision-00-enable-demo-modules.sh` | Stands up the demo site in the `local`, `ci`, `dev` and `stage` environments: applies the `recipes/page` content model, sets the site name, installs the contrib, service and custom site modules (including core's `media` and `navigation`), and runs their deploy hooks |
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** No settings override.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** `settings.config_split.php` enables the split matching the detected environment - `local`, `ci`, `dev` or `stage` - so each environment imports only its own set.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`, after the core Navigation module replaces the classic Toolbar.
- **Configured:** No settings override.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** `settings.xmlsitemap.php` stops cron regeneration and search engine submission outside production, so non-production environments are never advertised.
...
- **Configured:** `settings.seckit.php` disables the Content Security Policy and the upgrade-insecure-requests header locally and in CI, where the site is not served over HTTPS.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** `settings.shield.php` forces Shield on in non-production environments other than `local` and `ci`, where it is forced off. Production is left untouched, so Shield stays under UI control there.
...
[ClamAV](https://www.drupal.org/project/clamav) scans uploaded files for malware with the ClamAV engine before they are saved.

- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`; ships only when the ClamAV service is selected.
- **Configured:** `settings.clamav.php` selects daemon or executable mode and points the daemon at the ClamAV service, but only when `DRUPAL_CLAMAV_ENABLED` is set.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** `settings.environment_indicator.php` names the indicator after the detected environment and colors it per environment - red for production, yellow for stage, green for dev - with the toolbar integration and favicon marker turned on.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** `settings.reroute_email.php` enables rerouting in every environment except `local`, `ci`, `stage` and `prod`.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** `settings.robotstxt.php` serves a `Disallow: /` file outside production, so non-production environments are not indexed.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`.
- **Configured:** `settings.stage_file_proxy.php` sets the origin from `DRUPAL_STAGE_FILE_PROXY_ORIGIN` outside production and injects the Shield credentials into that URL when they are set, so a shielded origin still serves files.

### `redis`

[Redis](https://www.drupal.org/project/redis) integrates Drupal with the Redis in-memory store for cache and lock backends.

- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`; ships only when the Redis service is selected.
- **Configured:** `settings.redis.php` makes Redis the default cache backend, registers the module container YAML files and swaps the bootstrap container over - all gated on `DRUPAL_REDIS_ENABLED` and on the `redis` PHP extension being loaded, so a two-stage deployment can provision the service before switching the cache.
...
- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`, alongside the Solr backend.
- **Configured:** No settings override.

### `search_api_solr`

[Search API Solr](https://www.drupal.org/project/search_api_solr) is a Search API backend that integrates Apache Solr for fast, scalable search.

- **Installed:** During provisioning by `scripts/provision-00-enable-demo-modules.sh`; ships only when the Solr service is selected.
- **Configured:** No settings override.
...
- **Installed:** During development provisioning by `scripts/provision-10-enable-dev-modules.sh`.
- **Configured:** `settings.devel.php` excludes it from the exported configuration.
...
- **Installed:** During development provisioning by `scripts/provision-10-enable-dev-modules.sh` with `GENERATED_CONTENT_CREATE=1`, unless `DRUPAL_GENERATED_CONTENT_SKIP=1` installs it without generating.
- **Configured:** `settings.generated_content.php` excludes it from the exported configuration.
...
- **Installed:** During development provisioning by `scripts/provision-10-enable-dev-modules.sh`, and as a dependency of the `ys_demo` demo module.
- **Configured:** `settings.testmode.php` excludes it from the exported configuration.
...
- **Installed:** As a dependency of the `ys_migrate` demo migration module, which `scripts/provision-20-migration.sh` installs during provisioning.
- **Configured:** No settings override.

### `migrate_tools`

[Migrate Tools](https://www.drupal.org/project/migrate_tools) provides Drush commands and a UI to run, roll back and monitor migrations. `scripts/provision-20-migration.sh` drives it during provisioning.
...
➡️ See [Development](README.mdx) for more details on how to work
...
`scripts/provision-10-enable-dev-modules.sh` installs it alongside the other
