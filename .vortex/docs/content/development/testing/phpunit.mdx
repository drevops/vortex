...
is running. The number of custom modules multiplied by the number of
environment types makes it likely that certain settings get missed. Missed
settings may lead to unexpected issues when deploying a project to a different
environment.

They're intended to be used in your site and kept up-to-date with the
...
to check that the continuous integration configuration is correct. They're
...
and tags before they're deployed to the hosting environment.
...
By default, PHPUnit runs tests for custom modules and themes, Drupal
...
Sometimes you need to ignore lines from coverage. For example, when testing a
module that uses a third-party library, you don't need to test the library
itself.
