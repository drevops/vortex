...
**Vortex** comes with a [pre-configured Jest setup](https://github.com/drevops/vortex/blob/main/jest.config.js).
...
<Tabs>
  ...
  <TabItem value="docker-compose" label="Docker Compose">
    ```shell
    docker compose exec cli bash -c "npm test"      # Run all Jest tests.
    ```
  </TabItem>
</Tabs>
...
`ahoy test-js` doesn't forward extra arguments, so run subsets through
`ahoy cli`:

<Tabs>
  <TabItem value="ahoy" label="Ahoy" default>
    ```shell
    ahoy cli "npm test -- ys_demo"
    ```
  </TabItem>
  <TabItem value="docker-compose" label="Docker Compose">
    ```shell
    docker compose exec cli bash -c "npm test -- ys_demo"
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem value="ahoy" label="Ahoy" default>
    ```shell
    ahoy cli "npm test -- -t 'should increment the value'"
    ```
  </TabItem>
  <TabItem value="docker-compose" label="Docker Compose">
    ```shell
    docker compose exec cli bash -c "npm test -- -t 'should increment the value'"
    ```
  </TabItem>
</Tabs>
...
This boilerplate test runs in the continuous integration pipeline when you
...
By default, Jest discovers and runs test files with the `.test.js` extension
in `web/modules/custom/*/js/` directories and their subdirectories.
...
In the continuous integration pipeline, Jest runs on the first container only,
see [Test parallelism](../../continuous-integration/README.mdx#test-parallelism).
...
Set the `VORTEX_CI_JEST_IGNORE_FAILURE` environment variable to `1` to ignore
