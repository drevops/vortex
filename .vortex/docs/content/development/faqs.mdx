...
<Tabs>
  ...
  <TabItem value="docker-compose" label="Docker Compose">
    1. Run `XDEBUG_ENABLE=true docker compose up -d cli appserver webserver`.
    2. Enable listening for incoming debug connections in your IDE.
    3. If required, provide server URL to your IDE as it appears in the browser.
    4. Enable Xdebug flag in the request coming from your web browser (use one of
       the extensions or add `?XDEBUG_SESSION_START=1` to your URL).
    ...
    To disable, run `docker compose up -d cli appserver webserver`.
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem value="docker-compose" label="Docker Compose">
    1. Enable debugging: `XDEBUG_ENABLE=true docker compose up -d cli appserver webserver`
...
fail. The browser container randomly gets stuck for an unknown reason.

Restart the browser container: `docker compose restart browser`
