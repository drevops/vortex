...
The CI security audit workflow runs `npm audit` against the root and the theme dependency trees, failing on advisories rated `high` or above. When one appears:
...
3. **Pin a patched transitive version**: when the affected package is not a direct dependency, add an `overrides` entry for it in `package.json` and run `npm install`. `npm audit` has no per-advisory ignore list, so this is how a specific advisory leaves the tree.
