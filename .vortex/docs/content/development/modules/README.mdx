...
**Vortex** isn't a Drupal distribution: it ships no installation profile, and
...
Deselecting one removes its Composer requirement, its settings override, its
line in the provisioning script, its Behat feature, and its settings test
...
| Development provisioning | [`scripts/provision-10-enable-dev-modules.sh`](https://github.com/drevops/vortex/blob/main/scripts/provision-10-enable-dev-modules.sh) | Installs the modules that never reach production - the development tools and the email rerouting safety net - in the same 4 environments. ➡️ See [Provision](../provision.mdx#development-modules) |
...
module shipped that way. Every other module in
[Contributed modules](contributed-modules.mdx) is installed by one of the
preceding mechanisms, so selecting the module is enough to get it enabled.
