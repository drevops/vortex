...
in the project root is the primary configuration for AI coding agents. It's
...
- **Critical rules** - constraints that agents must always follow (for example,
  never modify `vendor/drevops/vortex-tooling/src/`, always export config after
  admin UI changes).
...
commands. This lets an agent build, provision, lint, and test through `ahoy`
...
project-specific task, such as a release checklist or a legacy content import
procedure.
...
`.claude/settings.json`, so a skill is shared with the team as soon as it's
