...
<Tabs>
  ...
  <TabItem value="docker-compose" label="Docker Compose">
    ```shell
    # Run all Jest tests with coverage.
    docker compose exec cli yarn test
    # Run test files matching a pattern.
    docker compose exec cli yarn test ys_demo
    # Run tests matching a name.
    docker compose exec cli yarn test -t 'should increment the value'
    ```
  </TabItem>
</Tabs>
...
`ahoy cli "yarn test ..."` as shown above.
...
directories and their subdirectories - configured in [`jest.config.js`](https://github.com/drevops/vortex/blob/main/jest.config.js).
Adding a new module with tests requires no configuration changes.
