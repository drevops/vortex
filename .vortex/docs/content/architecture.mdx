...
- **Explicit logging helps**: Scripts log every major step, so you can follow
  what's going on.
...
The scripts aren't part of the template's own files: they're a dependency.
...
holds the minor version. As a result, a new minor release can't reach your
deployments until you raise the constraint yourself. Once installed, every
script is available as a Composer binary at `vendor/bin/vortex-<script>` - for
example `vendor/bin/vortex-provision` or `vendor/bin/vortex-deploy` - and the
Ahoy commands wrap those binaries.
...
variables, and add project logic as your own scripts under `scripts/` (the
`provision` script discovers and runs `scripts/provision-*.sh`). Reach for a
...
- The environment type (for example, local, CI, stage) is automatically detected
  based on the environment provider settings located in `web/sites/default/includes/providers/`.
...
**Vortex** also includes tests for these settings to ensure they're loaded
...
**Vortex** supports Acquia and Lagoon hosting out of the box. These integrations:
