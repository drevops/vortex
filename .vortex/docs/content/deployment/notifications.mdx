...
When a deployment runs, **Vortex** captures the `vortex-provision` output to a
log file and exposes it to notifications through the `VORTEX_NOTIFY_LOG`
variable, which the hosting integrations set automatically. The **email**
channel appends this log to the message body via the `%deployment_log%` token,
so recipients can see exactly what happened during the deploy.

The section is omitted when no log is available. By default only the trailing
100 lines are included, to keep the email a reasonable size; adjust this with
`VORTEX_NOTIFY_EMAIL_LOG_LINES` (set to `0` to include the entire log).

| Variable | Required | Default | Location | Description |
|----------|----------|---------|----------|-------------|
| `VORTEX_NOTIFY_LOG` | No | Provision log path | Hosting | Deployment log file included in the notification body |
...
| `VORTEX_NOTIFY_EMAIL_LOG` | No | `VORTEX_NOTIFY_LOG` | Hosting | Deployment log file to include in the body |
| `VORTEX_NOTIFY_EMAIL_LOG_LINES` | No | `100` | `.env` | Max trailing log lines to include (`0` = all) |
