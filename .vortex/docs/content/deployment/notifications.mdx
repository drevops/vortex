...
`VORTEX_PROVISION_LOG=1`, truncated fresh each run - and when
`VORTEX_NOTIFY_LOG=1` the notifier collects every log in that directory, each as
its own titled section, and the **email**, **Slack**, **JIRA**, and **webhook**
channels add them to their message. The email and webhook message templates
expose the collected logs as the `%deployment_log%` token; Slack adds them to the
message attachment and JIRA as a comment code block. Any future script that drops
a `<name>.log` into the directory is collected automatically.
...
Logs are written to `VORTEX_NOTIFY_LOG_DIR` (default `./.logs`).
...
| `VORTEX_NOTIFY_LOG_DIR` | No | `./.logs` | `.env` | Directory the logs are collected from |
