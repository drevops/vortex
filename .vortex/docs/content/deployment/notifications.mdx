...
| `%deployment_log%` | Full deployment log, when enabled | |
...
Set `VORTEX_NOTIFY_LOG=1` (in `.env` or per environment) to include the full
deployment log in notifications. When enabled, `vortex-provision` captures its
output to a log file during the deploy and the **email**, **Slack**, **JIRA**,
and **webhook** channels add it to their message, so recipients can see exactly
what happened during the deploy. The email and webhook message templates expose
it as the `%deployment_log%` token; Slack adds it to the message attachment and
JIRA as a comment code block.

:::warning

The deployment log is included verbatim. Make sure the `vortex-provision` output
does not print secrets, and enable `VORTEX_NOTIFY_LOG` only for the channels and
environments where exposing the full log is acceptable.

:::

Unless overridden, the log file is `/tmp/vortex-provision.log`; change it with
`VORTEX_NOTIFY_LOG_FILE`.

| Variable | Required | Default | Location | Description |
|----------|----------|---------|----------|-------------|
| `VORTEX_NOTIFY_LOG` | No | `0` | `.env` or Hosting | Set to `1` to include the deployment log in notifications |
| `VORTEX_NOTIFY_LOG_FILE` | No | `/tmp/vortex-provision.log` | `.env` | Deployment log file path |
