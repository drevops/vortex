...
The script deploys the code to a remote location by calling the relevant
scripts for each type of deployment. The `$VORTEX_DEPLOY_TYPES` variable defines
the types as a comma-separated list of one or multiple supported deployment
types:
...
and sends [notifications](notifications/README.mdx) - that part runs on the hosting
...
When a pull request is raised against one of the preceding branches, a
deployment runs for the pull request's branch. Depending on the hosting
platform, this may create a temporary environment named after the pull request.
...
features or projects spanning multiple pull requests. An example is a
migration branch whose code and configuration aren't ready to merge into
...
reaches an environment only when it's explicitly labeled for it. This is
...
labels aren't available natively, so populate `$VORTEX_DEPLOY_PR_LABELS`
...
| [Notifications](notifications/README.mdx) | Send deployment notifications |
