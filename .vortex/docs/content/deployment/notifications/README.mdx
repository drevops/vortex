...
`VORTEX_PROVISION_LOG=1`, truncated fresh each run. When `VORTEX_NOTIFY_LOG=1`,
the notifier collects every log in that directory, each as its own titled section.
The **email**, **Slack**, **Jira**, and **webhook** channels add them to their
...
Deployment logs are included verbatim. Make sure producing scripts don't print
...
directory, make sure that location isn't readable by other users.
