...
downloads the Lagoon CLI and points it at the configured Lagoon instance. Over
SSH, it requests a deployment for the current branch or pull request. Lagoon
then:
...
Lagoon doesn't support tag deployments - the script reports this and skips the
deployment.
...
3. Ensure your CI has the Lagoon SSH key configured for authentication - the
   CLI connects over SSH, so a missing key fails the deployment.
...
`develop` branches. It treats an environment as production when Lagoon marks
