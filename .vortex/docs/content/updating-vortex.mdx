...
Updating **Vortex** gives existing projects:
...
3. Review the changes made by the update and selectively add them.
   ...
   Where the update replaced a file your project had changed, `.logs/vortex-update.md` records the replaced change alongside the change the new version brings. Work through it to re-apply your changes, then delete the entries you have resolved. The file isn't tracked in Git.
   ...
   your project's versions, then updating the package versions in your
   `composer.json` manually. After settling the constraints, run
   `ahoy composer update` and commit the regenerated `composer.lock`, so the
   build installs the versions you selected rather than the stale locked ones.
