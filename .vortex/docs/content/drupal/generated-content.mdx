...
`scripts/provision-10-enable-dev-modules.sh` in non-production environments,
passing `GENERATED_CONTENT_CREATE=1` so the content is created as part of the
install. Setting `DRUPAL_GENERATED_CONTENT_SKIP=1` installs the module without
the content.
