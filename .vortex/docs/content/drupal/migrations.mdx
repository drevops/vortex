...
- Disabling the search indexes while the migrations run
...
On projects that also use the Solr service, the script disables the Search API
indexes that are enabled before the rollback and the imports, then enables those
same indexes again once the migrations finish. An index that is already disabled
is left alone, and a site with no enabled indexes skips the step entirely.

Without this, every migrated entity is sent to the search server as it is
created, which slows the migration down for no benefit - the
`scripts/provision-30-search-index.sh` script rebuilds the index from scratch
immediately afterward.

The indexes are also restored when a migration fails part-way through, and the
migration failure is still reported. If the restore itself fails, provisioning
fails with `Failed to enable search indexes.` and the indexes stay disabled -
re-run `ahoy provision`, or enable them with `drush search-api:enable <index>`.
