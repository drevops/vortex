...
5. Ensure the `ys_migrate` module (with its `MigrateContent` plugin) is present.
...
The migration runs as a `MigrateContent` persistent deploy plugin in the
`ys_migrate` module (`src/Plugin/PersistentDeploy/MigrateContent.php`). It runs
on every `drush deploy:hook` in non-production environments - so it runs during
`ahoy provision` and on every deploy. See [Deployment](../deployment) for how
persistent deploy plugins work.
...
4. Add your migration IDs to the `MigrateContent` plugin's `MIGRATIONS` constant:
