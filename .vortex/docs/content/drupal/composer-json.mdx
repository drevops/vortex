...
- [`post-install-cmd`](https://getcomposer.org/doc/articles/scripts.md#post-install-cmd):
  ...
  executed, which creates the required directories for the Drupal
  installation.
- [`post-update-cmd`](https://getcomposer.org/doc/articles/scripts.md#post-update-cmd):
  ...
  same `createRequiredFiles` method as in `post-install-cmd`, ensuring the
  required directories are present following an update of dependencies.
