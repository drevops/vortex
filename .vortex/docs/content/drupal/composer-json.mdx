...
- [`drupal/sdc_devel`](https://www.drupal.org/project/sdc_devel): Provides
  development tooling for Single Directory Components (SDC), including a
  `drush sdc-devel:validate` command that checks component schemas and Twig
  templates against best practices. Run it via `ahoy lint-sdc`.
