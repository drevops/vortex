...
only when `VORTEX_PROVISION_OVERRIDE_DB` is set to `1` - that is, when the
database has just been imported or installed from scratch. Re-provisioning a site
whose content is preserved keeps that content untouched. Content is also skipped
when the site already holds generated content, so repeated provisioning never
duplicates it.
