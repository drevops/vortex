...
Projects that also run migrations have the search indexes disabled for the
duration of `scripts/provision-20-migration.sh`, so migrated entities are not
sent to the search server one at a time while the index is about to be rebuilt.
See [Migrations](./migrations#search-indexing).
