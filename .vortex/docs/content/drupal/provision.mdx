...
Environment names are matched exactly, so every environment that should run the
script has to be listed. An environment named `devops` or `stage-uat` will not
match `dev` or `stage` - add it to the list explicitly.
