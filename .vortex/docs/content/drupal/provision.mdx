...
| `provision-15-generated-content.sh`   | Generates content from the Generated content plugins     |
...
Projects with the
[Generated content](https://www.drupal.org/project/generated_content) module ship
with a `scripts/provision-15-generated-content.sh` custom script that creates
content from every generated content plugin the site provides.

The module creates content from its own `hook_modules_installed()`, which only
fires while the module providing the plugins is being installed. The script
calls the generated content repository directly instead, so content is created
even when the modules were installed earlier in the provisioning run.

The script runs only in the `local`, `ci`, `dev` and `stage` environments, and
only when [`$VORTEX_PROVISION_OVERRIDE_DB`](../development/variables#vortex_provision_override_db)
is set to `1` - that is, when the database has just been imported or installed
from scratch. Re-provisioning a site whose content is preserved keeps that
content untouched. Content is also skipped when the site already holds generated
content, so repeated provisioning never duplicates it.
