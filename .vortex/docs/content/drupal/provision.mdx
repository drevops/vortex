...
create content from every generated content plugin the site provides in the same
`scripts/provision-10-enable-dev-modules.sh` custom script, after the development
modules are installed and before the search index is rebuilt.

The module creates content from its own `hook_modules_installed()`, which only
fires while the module providing the plugins is being installed. The script calls
the generated content repository directly instead, so content is created even
when the modules were installed earlier in the provisioning run.

Content is generated only when `VORTEX_PROVISION_OVERRIDE_DB` is set to `1` -
that is, when the database has just been imported or installed from scratch.
Re-provisioning a site whose content is preserved keeps that content untouched.
Generation is also skipped when the site already holds generated content, so
repeated provisioning does not duplicate it. The module only records entities
from plugins that declare `tracking: TRUE`, which is the default, so entities
from untracked plugins are created again on every fresh database.

Generation is heavier than the rest of provisioning, so it runs without a PHP
memory limit.

To opt out of content generation, set `DRUPAL_GENERATED_CONTENT_SKIP=1` in the
`.env` file or in your hosting provider's specific environment.
