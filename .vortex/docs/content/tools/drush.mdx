...
environment and can be used as follows:
...
Note that your SSH key must be loaded in your `ssh-agent` for Drush SSH client
to connect to the remote environment:

```shell
ssh-add ~/.ssh/path/to/key             # Run on host to run remote Drush commands from host.
# or
pygmy addkey --key ~/.ssh/path/to/key  # Run on the host to execute remote Drush commands from inside the container (including ahoy drush).
```
