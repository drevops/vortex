...
final class VideoRecorder {
  ...
  /** Poster timestamp for the first recording (captures the opening frame). */
  ...
  /**
   * Install non-interactively into `$workspace/star_wars`.
   *
   * @param string $uri
   *   `--uri` value (typically the project_root so that the in-development
   *   template is used as the source).
   *
   * @return string
  ...
  public function runInstall(string $workspace, string $uri): string {
    ...
    $this->info('Installing non-interactively');
    $this->note("URI: $uri");

    $env = [
      'VORTEX_PROMPT_BUILD_NOW' => '0',
      'VORTEX_NAME' => 'Star Wars',
      'VORTEX_ORG' => 'Rebellion',
    ];
   ...
   *     a spawned session where asciinema echoes the spawn
   *     command on event 1).
