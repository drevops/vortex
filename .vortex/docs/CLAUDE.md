...
Six terminal demo videos live in `static/img/` (`install.*`, `build.*`,
...
1. Runs the CLI non-interactively (or via `expect` when `install` is in the requested set) using `--uri=<project_root>`, producing `$workspace/star_wars`.
