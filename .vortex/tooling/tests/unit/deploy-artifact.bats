...
@test "Missing or Invalid VORTEX_DEPLOY_TYPES" {
  ...
  assert_output_contains "Missing required value for VORTEX_DEPLOY_TYPES. Must be a combination of comma-separated values (to support multiple deployments): code, webhook, lagoon."
...
@test "Check setting and default values for required variables" {
  ...
  run .vortex/tooling/src/deploy-artifact
  assert_failure
