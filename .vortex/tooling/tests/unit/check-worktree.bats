#!/usr/bin/env bats
##
# Unit tests for the 'vortex-check-worktree' script.
#
# shellcheck disable=SC2030,SC2031

load ../_helper.bash

@test "check-worktree: clean working tree passes" {
...
@test "check-worktree: modified tracked file fails" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
  ...
  assert_output_contains "Add paths that are generated during the build and are not committed to VORTEX_CHECK_WORKTREE_IGNORE."
  assert_output_contains "The build left changes in the working tree."

  popd >/dev/null
}

@test "check-worktree: untracked file fails" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "check-worktree: file ignored by Git passes" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "check-worktree: shipped ignore list covers the paths written by the pipeline" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "check-worktree: custom ignore list excludes a path" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1

  export VORTEX_CHECK_WORKTREE_IGNORE="build-manifest.json"
  touch build-manifest.json
...
@test "check-worktree: custom ignore list replaces the shipped one" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "check-worktree: blank entries in the ignore list are skipped" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "check-worktree: ignore list of separators only checks every path" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1

  export VORTEX_CHECK_WORKTREE_IGNORE=","
  echo "# Processed to run in CI." >>docker-compose.yml
...
@test "check-worktree: fails outside a Git repository" {
  cp "${LOCAL_REPO_DIR}/.env" "${CURRENT_PROJECT_DIR}/.env"

  run "${LOCAL_REPO_DIR}/.vortex/tooling/src/vortex-check-worktree"
  assert_failure
