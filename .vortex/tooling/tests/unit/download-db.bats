...
@test "download-db: Download with URL source" {
  ...
  cat >.vortex/tooling/src/download-db-url <<'EOF'
#!/usr/bin/env bash
...
EOF
  chmod +x .vortex/tooling/src/download-db-url
...
@test "download-db: Container registry source skips file existence check" {
  ...
  mkdir -p .vortex/tooling/src
  cat >.vortex/tooling/src/download-db-container-registry <<'EOF'
#!/usr/bin/env bash
echo "Started database data container image download."
echo "Finished database data container image download."
EOF
  chmod +x .vortex/tooling/src/download-db-container-registry
