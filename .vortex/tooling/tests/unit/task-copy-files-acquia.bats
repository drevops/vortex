#!/usr/bin/env bats
##
# Unit tests for the 'vortex-task-copy-files-acquia' script.
#
...
setup_variables() {
  export VORTEX_TASK_COPY_FILES_ACQUIA_KEY="test-key"
  export VORTEX_TASK_COPY_FILES_ACQUIA_SECRET="test-secret"
  export VORTEX_TASK_COPY_FILES_ACQUIA_APP_NAME="testapp"
  export VORTEX_TASK_COPY_FILES_ACQUIA_SRC="dev"
  export VORTEX_TASK_COPY_FILES_ACQUIA_DST="test"
  export VORTEX_TASK_COPY_FILES_ACQUIA_STATUS_RETRIES="1"
  export VORTEX_TASK_COPY_FILES_ACQUIA_STATUS_INTERVAL="0"
}

@test "task-copy-files-acquia: copies the files between environments" {
  ...
  declare -a STEPS=(
    ...
    "Copying files from dev to test environment."
...
@test "task-copy-files-acquia: reports the rejected credentials" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "task-copy-files-acquia: reports the missing application" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "task-copy-files-acquia: reports the missing source environment" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "task-copy-files-acquia: reports the missing destination environment" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "task-copy-files-acquia: missing key" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1

  mock_curl=$(mock_command "curl")

  setup_variables
  unset VORTEX_TASK_COPY_FILES_ACQUIA_KEY
  unset VORTEX_ACQUIA_KEY

  run .vortex/tooling/src/vortex-task-copy-files-acquia
  assert_failure

  assert_output_contains "Missing required value for VORTEX_TASK_COPY_FILES_ACQUIA_KEY or VORTEX_ACQUIA_KEY."
