...
@test "fetch-db-ftp: Download database file successfully" {
...
@test "fetch-db-ftp: Fail when VORTEX_DOWNLOAD_DB_FTP_USER is missing" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "fetch-db-ftp: Fail when VORTEX_DOWNLOAD_DB_FTP_PASS is missing" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "fetch-db-ftp: Fail when VORTEX_DOWNLOAD_DB_FTP_HOST is missing" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "fetch-db-ftp: Fail when VORTEX_DOWNLOAD_DB_FTP_PORT is missing" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
...
@test "fetch-db-ftp: Fail when VORTEX_DOWNLOAD_DB_FTP_FILE is missing" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
