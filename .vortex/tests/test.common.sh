...
bats() {
  ...
  if [ -n "${VORTEX_DEV_TEST_COVERAGE_DIR:-}" ]; then
    ...
    kcov --include-pattern=.sh,.bash --bash-parse-files-in-dir="${SCRIPTS_DIR}/bats","${TEST_DIR}" --exclude-pattern=vendor,node_modules "${VORTEX_DEV_TEST_COVERAGE_DIR}" "${TEST_DIR}/node_modules/.bin/bats" "$@"
  else
