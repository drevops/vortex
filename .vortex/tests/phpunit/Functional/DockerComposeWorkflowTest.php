...
class DockerComposeWorkflowTest extends FunctionalTestCase {
  ...
  public function testDockerComposeWorkflowFull(): void {
    ...
    $this->cmd('docker compose exec -T cli bash -cl "npm ci"', txt: 'Install development dependencies with npm', tio: 10 * 60);
    ...
    $this->cmd('docker compose exec -T cli npm run lint', txt: 'Lint code with module linters', tio: 10 * 60);
    $this->cmd('docker compose exec -T cli bash -cl "npm run --prefix=\${WEBROOT}/themes/custom/\${DRUPAL_THEME} lint"', txt: 'Lint code with theme linters', tio: 10 * 60);
  ...
  public function testDockerComposeWorkflowNoTheme(): void {
    static::$sutPrompts = ['theme' => 'olivero'];
    $this->prepareSut();
