...
:::info
    ...
    The `$DREVOPS_PROVISION_ENVIRONMENT` environment variable can be utilized
...
**Scaffold** provides a [set of unit tests](https://github.com/drevops/scaffold/blob/develop/tests/phpunit/Drupal) that
...
After installing **Scaffold**, run `vendor/bin/phpunit --group=drupal_settings` to
run the tests for the settings provided by **Scaffold**.
