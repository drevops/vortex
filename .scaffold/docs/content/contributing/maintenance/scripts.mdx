...
16. MAY rely on variables from the external scripts (not prefixed with a
    ...
    the file.
17. MAY call other **Scaffold** scripts (discouraged), but MUST source them rather
...
Follow these guidelines when creating or updating **Scaffold** variables.

1. Local variables MUST be in lowercase, and global variables MUST be in
   uppercase.

2. All **Scaffold** variables MUST start with `DREVOPS_` to separate DrevOps from
...
4. **Scaffold** action-specific script variables MUST be scoped within their own
...
5. Drupal-related variables SHOULD start with `DRUPAL_` and SHOULD have a module
   name added as a second prefix. This is to separate **Scaffold**,
...
Use the boilerplate script below to kick-start your custom **Scaffold** script.
