#!/usr/bin/env bats
#
# Test for update Scaffold functionality.
#
...
@test "Update" {
  # Add custom files
  touch "test1.txt"
  # File resides in directory that is included in Scaffold when initialised.
  mkdir -p ".docker"
  ...
  git_add_all_commit "Init Scaffold"

  # Assert that custom file preserved.
  assert_file_exists "test1.txt"
  # Assert that custom file in a directory used by Scaffold is preserved.
  assert_file_exists ".docker/test2.txt"

  # Assert no changes were introduced.
  assert_git_clean

  # Releasing new version of Scaffold (note that installing from the local tag
  ...
  latest_commit=$(git_commit "New version of Scaffold" "${LOCAL_REPO_DIR}")

  # Override Scaffold release commit in .env file.
  echo DREVOPS_INSTALL_COMMIT="${latest_commit}" >>.env
  # Enforce debugging of the install script.
  export DREVOPS_INSTALL_DEBUG=1
  # Override install script with currently tested one to be called from ./scripts/drevops/update-scaffold.sh
  export DREVOPS_INSTALLER_URL="file://${INSTALLER_CHECKOUT_DIR}/install.php"
  # shellcheck disable=SC2059
  run ahoy update-scaffold
  assert_success
  assert_files_present
  assert_git_repo
