#!/usr/bin/env bats
#
# Tests for scripts/drevops/deploy-container-registry.sh script.
#
...
@test "Missing DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP - deployment should not proceed" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1

  unset DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP

  run scripts/drevops/deploy-container-registry.sh
  assert_success
  assert_output_contains "Services map is not specified in DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP variable. Container registry deployment will not continue."

  popd >/dev/null
}

@test "Container registry deployment with valid DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1

  export DREVOPS_DEPLOY_CONTAINER_REGISTRY="registry.example.com"
  export DREVOPS_DEPLOY_CONTAINER_REGISTRY_IMAGE_TAG="test_latest"
  provision_docker_config_file DREVOPS_DEPLOY_CONTAINER_REGISTRY

  export DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP="service1=image1,service2=image2,service3=image3"
...
@test "Container registry deployment with services not running" {
  ...
  export DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP="service1=image1"
...
@test "Invalid DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP provided" {
  pushd "${LOCAL_REPO_DIR}" >/dev/null || exit 1
  ...
  # No key/value pair
  export DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP="service1"
  ...
  export DREVOPS_DEPLOY_CONTAINER_REGISTRY_MAP="service1=image1 service2=image2"
