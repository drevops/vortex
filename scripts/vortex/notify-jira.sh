...
message=$(php -r "echo str_replace('%project%', '${VORTEX_NOTIFY_JIRA_PROJECT}', '${message}');")
message=$(php -r "echo str_replace('%label%', '${VORTEX_NOTIFY_JIRA_LABEL}', '${message}');")
message=$(php -r "echo str_replace('%timestamp%', '${timestamp}', '${message}');")
message=$(php -r "echo str_replace('%environment_url%', '${VORTEX_NOTIFY_JIRA_ENVIRONMENT_URL}', '${message}');")
message=$(php -r "echo str_replace('%login_url%', '${VORTEX_NOTIFY_JIRA_LOGIN_URL}', '${message}');")

# Build JIRA Atlassian Document Format (ADF) from the message.
# For simplicity, we'll build a basic ADF with the label as code and URLs as links.
comment_body=$(php -r "
\$content = array(
  array(
    'type' => 'paragraph',
    'content' => array(
      array('type' => 'text', 'text' => '## This is an automated message ##'),
      array('type' => 'hardBreak'),
      array('type' => 'hardBreak'),
      array('type' => 'text', 'text' => 'Site ${VORTEX_NOTIFY_JIRA_PROJECT} '),
      array('type' => 'text', 'text' => '${VORTEX_NOTIFY_JIRA_LABEL}', 'marks' => array(array('type' => 'code'))),
      array('type' => 'text', 'text' => ' has been deployed at ${timestamp} and is available at '),
      array('type' => 'text', 'text' => '${VORTEX_NOTIFY_JIRA_ENVIRONMENT_URL}', 'marks' => array(array('type' => 'link', 'attrs' => array('href' => '${VORTEX_NOTIFY_JIRA_ENVIRONMENT_URL}')))),
      array('type' => 'text', 'text' => '.'),
      array('type' => 'hardBreak'),
      array('type' => 'hardBreak'),
      array('type' => 'text', 'text' => 'Login at: '),
      array('type' => 'text', 'text' => '${VORTEX_NOTIFY_JIRA_LOGIN_URL}', 'marks' => array(array('type' => 'link', 'attrs' => array('href' => '${VORTEX_NOTIFY_JIRA_LOGIN_URL}'))))
    )
  )
);
