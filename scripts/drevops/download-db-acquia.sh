...
echo "[INFO] Started database dump download from Acquia."
...
if [ -f "${file_name_discovered}" ] ; then
  ...
  if [ ! -f "${file_name_compressed}" ] ; then
    ...
    echo "  [OK] Found existing cached gzipped DB file ${file_name_compressed} for DB ${DREVOPS_DB_DOWNLOAD_ACQUIA_DB_NAME}."
  fi
...
echo "  [OK] Finished database dump download from Acquia."
