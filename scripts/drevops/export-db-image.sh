...
DREVOPS_DB_EXPORT_IMAGE_ARCHIVE_FILE="${DREVOPS_DB_EXPORT_IMAGE_ARCHIVE_FILE:-${1}}"

# Container image to store in a form of `<org>/<repository>`.
DREVOPS_DB_EXPORT_IMAGE="${DREVOPS_DB_EXPORT_IMAGE:-}"

# Container registry name.
DREVOPS_DB_EXPORT_CONTAINER_REGISTRY="${DREVOPS_DB_EXPORT_CONTAINER_REGISTRY:-${DREVOPS_CONTAINER_REGISTRY:-docker.io}}"

# The service name to capture.
DREVOPS_DB_EXPORT_SERVICE_NAME="${DREVOPS_DB_EXPORT_SERVICE_NAME:-mariadb}"

# Directory with database image archive file.
DREVOPS_DB_EXPORT_IMAGE_DIR="${DREVOPS_DB_EXPORT_IMAGE_DIR:-${DREVOPS_DB_DIR}}"
...
[ -z "${DREVOPS_DB_EXPORT_IMAGE}" ] && fail "Destination image name is not specified. Please provide container image as a variable DREVOPS_DB_EXPORT_IMAGE in a format <org>/<repository>." && exit 1

cid="$(docker compose ps -q "${DREVOPS_DB_EXPORT_SERVICE_NAME}")"
note "Found ${DREVOPS_DB_EXPORT_SERVICE_NAME} service container with id ${cid}."

new_image="${DREVOPS_DB_EXPORT_CONTAINER_REGISTRY}/${DREVOPS_DB_EXPORT_IMAGE}"

note "Locking and unlocking tables before upgrade."
docker compose exec -T "${DREVOPS_DB_EXPORT_SERVICE_NAME}" mysql -e "FLUSH TABLES WITH READ LOCK;"
sleep 5
docker compose exec -T "${DREVOPS_DB_EXPORT_SERVICE_NAME}" mysql -e "UNLOCK TABLES;"

note "Running forced service upgrade."
docker compose exec -T "${DREVOPS_DB_EXPORT_SERVICE_NAME}" sh -c "mysql_upgrade --force"
...
mkdir -p "${DREVOPS_DB_EXPORT_IMAGE_DIR}"
...
archive_file=$([ "${DREVOPS_DB_EXPORT_IMAGE_ARCHIVE_FILE}" ] && echo "${DREVOPS_DB_EXPORT_IMAGE_DIR}/${DREVOPS_DB_EXPORT_IMAGE_ARCHIVE_FILE//.sql/.tar}" || echo "${DREVOPS_DB_EXPORT_IMAGE_DIR}/export_db_$(date +%Y%m%d_%H%M%S).tar")
