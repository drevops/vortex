...
# The FTP port.
DREVOPS_DB_FTP_PORT="${DREVOPS_DB_FTP_PORT:-}"

# The file name, including any directories.
DREVOPS_DB_FTP_FILE="${DREVOPS_DB_FTP_FILE:-}"
...
[ -z "${DREVOPS_DB_FTP_PORT}" ] && echo "Missing required value for DREVOPS_DB_FTP_PORT." && exit 1
[ -z "${DREVOPS_DB_FTP_FILE}" ] && echo "Missing required value for DREVOPS_DB_FTP_FILE." && exit 1

curl -u "${DREVOPS_DB_FTP_USER}":"${DREVOPS_DB_FTP_PASS}" "ftp://${DREVOPS_DB_FTP_HOST}:${DREVOPS_DB_FTP_PORT}/${DREVOPS_DB_FTP_FILE}" -o "${DREVOPS_DB_DIR}/${DREVOPS_DB_FILE}"
