...
AC_API_FILES_SRC_ENV="${AC_API_FILES_SRC_ENV:-}"
# Destination environment name to copy to.
AC_API_FILES_DST_ENV="${AC_API_FILES_DST_ENV:-}"
...
[ -z "${AC_API_FILES_SRC_ENV}" ] && echo "ERROR: Missing value for AC_API_FILES_SRC_ENV." && exit 1
[ -z "${AC_API_FILES_DST_ENV}" ] && echo "ERROR: Missing value for AC_API_FILES_DST_ENV." && exit 1
...
ENVS_JSON=$(curl -s -L -H 'Accept: application/json, version=2' -H "Authorization: Bearer $TOKEN" "https://cloud.acquia.com/api/applications/${APP_UUID}/environments?filter=name%3D${AC_API_FILES_SRC_ENV}")
...
ENVS_JSON=$(curl -s -L -H 'Accept: application/json, version=2' -H "Authorization: Bearer $TOKEN" "https://cloud.acquia.com/api/applications/${APP_UUID}/environments?filter=name%3D${AC_API_FILES_DST_ENV}")
...
echo "==> Copying files from ${AC_API_FILES_SRC_ENV} to ${AC_API_FILES_DST_ENV} environment."
...
if [ "${TASK_COMPLETED}" == "0" ] ; then
  echo "==> Unable to copy files from $AC_API_FILES_SRC_ENV to $AC_API_FILES_DST_ENV environment."
  exit 1
fi
