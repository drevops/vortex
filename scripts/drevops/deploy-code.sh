...
DREVOPS_DEPLOY_CODE_SRC="${DREVOPS_DEPLOY_CODE_SRC:-}"

# The root directory where the deployment script should run from. Defaults to
# the current directory.
DREVOPS_DEPLOY_CODE_ROOT="${DREVOPS_DEPLOY_CODE_ROOT:-$(pwd)}"
...
# deployment key.
DREVOPS_DEPLOY_CODE_SSH_FINGERPRINT="${DREVOPS_DEPLOY_CODE_SSH_FINGERPRINT:-}"

# Default SSH file used if custom fingerprint is not provided.
DREVOPS_DEPLOY_CODE_SSH_FILE="${DREVOPS_DEPLOY_CODE_SSH_FILE:-${HOME}/.ssh/id_rsa}"

# Deployment report file name.
DREVOPS_DEPLOY_CODE_REPORT_FILE="${DREVOPS_DEPLOY_CODE_REPORT_FILE:-${DREVOPS_DEPLOY_CODE_ROOT}/deployment_report.txt}"

# Remote repository branch. Can be a specific branch or a token.
# @see https://github.com/drevops/git-artifact#token-support
DREVOPS_DEPLOY_CODE_GIT_BRANCH="${DREVOPS_DEPLOY_CODE_GIT_BRANCH:-[branch]}"
...
[ -z "${DREVOPS_DEPLOY_CODE_GIT_BRANCH}" ] && echo "Missing required value for DREVOPS_DEPLOY_CODE_GIT_BRANCH." && exit 1
[ -z "${DREVOPS_DEPLOY_CODE_SRC}" ] && echo "Missing required value for DREVOPS_DEPLOY_CODE_SRC." && exit 1
[ -z "${DREVOPS_DEPLOY_CODE_ROOT}" ] && echo "Missing required value for DREVOPS_DEPLOY_CODE_ROOT." && exit 1
[ -z "${DREVOPS_DEPLOY_CODE_REPORT_FILE}" ] && echo "Missing required value for DREVOPS_DEPLOY_CODE_REPORT_FILE." && exit 1
...
if [ -n "${DREVOPS_DEPLOY_CODE_SSH_FINGERPRINT}" ]; then
  echo "==> Custom deployment key is provided."
  DREVOPS_DEPLOY_CODE_SSH_FILE="${DREVOPS_DEPLOY_CODE_SSH_FINGERPRINT//:}"
  DREVOPS_DEPLOY_CODE_SSH_FILE="${HOME}/.ssh/id_rsa_${DREVOPS_DEPLOY_CODE_SSH_FILE//\"}"
fi

[ ! -f "${DREVOPS_DEPLOY_CODE_SSH_FILE}" ] && echo "ERROR: SSH key file ${DREVOPS_DEPLOY_CODE_SSH_FILE} does not exist." && exit 1

if ssh-add -l | grep -q "${DREVOPS_DEPLOY_CODE_SSH_FILE}"; then
  echo "==> SSH agent has ${DREVOPS_DEPLOY_CODE_SSH_FILE} key loaded."
else
  ...
  ssh-add -D > /dev/null
  ssh-add "${DREVOPS_DEPLOY_CODE_SSH_FILE}"
fi
...
cp -a "${DREVOPS_DEPLOY_CODE_ROOT}"/.git "${DREVOPS_DEPLOY_CODE_SRC}" || true
# Copying deployment .gitignore as it may not exist in deploy code source files.
cp -a "${DREVOPS_DEPLOY_CODE_ROOT}"/.gitignore.deployment "${DREVOPS_DEPLOY_CODE_SRC}" || true
...
"${HOME}/.composer/vendor/bin/robo" --ansi \
  --load-from "${HOME}/.composer/vendor/drevops/git-artifact/RoboFile.php" artifact "${DREVOPS_DEPLOY_CODE_GIT_REMOTE}" \
  --root="${DREVOPS_DEPLOY_CODE_ROOT}" \
  --src="${DREVOPS_DEPLOY_CODE_SRC}" \
  --branch="${DREVOPS_DEPLOY_CODE_GIT_BRANCH}" \
  --gitignore="${DREVOPS_DEPLOY_CODE_SRC}"/.gitignore.deployment \
  --report="${DREVOPS_DEPLOY_CODE_REPORT_FILE}" \
  --push
